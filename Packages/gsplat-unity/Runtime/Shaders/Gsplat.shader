...
Shader "Gsplat/Standard"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _HDRIntensityScale;

            bool _GammaToLinear;
            float _SplatSizeScale;
            int _SplatCount;
            ...
            SplatCovariance ReadCovariance(SplatSource source)
            {
                ...
                float3 scale = _ScaleBuffer[source.id] * _SplatSizeScale;
            ...
            v2f vert(appdata v)
            {
                ...
                o.vertex = center.proj + float4(corner.offset.x, _ProjectionParams.x * corner.offset.y, 0, 0)
                      + ClassicNoise(center.proj + _Time.y * _MovementSpeed) * _MovementScale;
